...
EXPECTED_LICENSE = 'A1B2C-3D4E5-F6G7H-8I9J0-K1L2M-3N4O5'
is_licensed = KEY_LICENSE == EXPECTED_LICENSE
logger.debug(f"License check: is_licensed={is_licensed}")
logger.debug(f"Environment variables: WP_SITE_URL={WP_SITE_URL}, WP_USERNAME={WP_USERNAME}, WP_APP_PASSWORD={'***' if WP_APP_PASSWORD else None}, COUNTRY={COUNTRY}, KEYWORD={KEYWORD}, FETCHER_TOKEN={'***' if FETCHER_TOKEN else None}, KEY_LICENSE={'***' if KEY_LICENSE else None}")
...
def save_company_to_wordpress(index, company_data, wp_headers):
    if not is_licensed:
        logger.info(f"Limited mode: Skipping company save for index {index}")
        return None, None
...
def scrape_job_details(job_url, auth_headers):
    ...
    try:
        ...
        if not is_licensed:
            logger.info("Limited mode: Clearing advanced fields")
            environment = ''
            level = ''
            job_functions = ''
            industries = ''
