...
EXPECTED_LICENSE_KEY = 'A1B2C-3D4E5-F6G7H-8I9J0-K1L2M-3N4O5'  # Expected license key for validation
...
WP_LICENSE_URL = f"{WP_SITE_URL}/wp-json/fetcher/v1/get-license"  # New endpoint for license key
...
logger.debug(f"WordPress endpoints: WP_URL={WP_URL}, WP_COMPANY_URL={WP_COMPANY_URL}, WP_MEDIA_URL={WP_MEDIA_URL}, WP_SAVE_COMPANY_URL={WP_SAVE_COMPANY_URL}, WP_SAVE_JOB_URL={WP_SAVE_JOB_URL}, WP_LICENSE_URL={WP_LICENSE_URL}")

def fetch_credentials_and_license():
    """Fetch WordPress credentials and license key from the REST API."""
    ...
    # Fetch and verify license key
    logger.info(f"Fetching license key from {WP_LICENSE_URL}")
    try:
        response = requests.get(WP_LICENSE_URL, timeout=5, verify=False)
        logger.debug(f"License request status: {response.status_code}, Response: {response.text[:200]}...")
        response.raise_for_status()
        data = response.json()
        logger.debug(f"License response: {json.dumps(data, indent=2)[:200]}...")
        if not data.get('success'):
            logger.error(f"Failed to fetch license key: {data.get('message', 'Unknown error')}")
            return False, None
        license_key = data.get('license_key')
        logger.debug(f"Fetched license_key={'***' if license_key else None}")
        if not license_key:
            logger.error("License key fetched but empty")
            return False, None
        if license_key != EXPECTED_LICENSE_KEY:
            logger.error("Invalid license key provided")
            return False, None
        logger.info("Successfully fetched and verified license key")
        return True, license_key
    except requests.exceptions.RequestException as e:
        logger.error(f"Failed to fetch license key from {WP_LICENSE_URL}: {str(e)}")
        return False, None
...
def get_or_create_term(term_name, taxonomy, wp_url, auth_headers):
    ...
    term_name = sanitize_text(text=term_name)
    if not term_name:
...
def main():
    ...
    is_valid, license_key = fetch_credentials_and_license()
    if not is_valid:
        logger.error("Cannot proceed without valid WordPress credentials or license key")
        print("Error: Cannot proceed without valid WordPress credentials or license key")
        return
